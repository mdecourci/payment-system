...
public final class Card {
    ...
    public Card(String customerId, CardBrand  brand,
        ...
        this.status        = expiry.isBefore(YearMonth.now())
                             ? CardStatus.EXPIRED : CardStatus.ACTIVE;
    }

    /** Constructor allowing explicit token — used for test cards. */
    public Card(String customerId, CardBrand  brand, String last4,
                YearMonth expiry, String cardholderName, String token) {
    ...
    public static Card testCard(String customerId, CardBrand  brand,
        ...
        return new Card(c.id, customerId, brand, c.maskedPan,
                        "tok_test_" + tokenSuffix, expiry, "Test Cardholder",
                        CardStatus.ACTIVE);
    }

    /** Full internal constructor. */
    private Card(String id, String customerId, CardBrand  brand,
                 String maskedPan, String token, YearMonth expiry,
                 String cardholderName, CardStatus status) {
        this.id             = id;
        this.customerId     = customerId;
        this.brand          = brand;
    ...
    // ── Business logic ────────────────────────────────────────────────────
    public void revoke() { status = CardStatus.REVOKED; }
    ...
    public CardBrand   brand()          { return brand; }
    public String           maskedPan()      { return maskedPan; }
    public String           token()          { return token; }
    public YearMonth        expiry()         { return expiry; }
    public String           cardholderName() { return cardholderName; }
    public CardStatus status()         { return status; }

    @Override
    public String toString() {
        return "Card[%s | %s %s exp=%s | %s]"
            .formatted(id, brand, maskedPan, expiry, status);
    }
}
