...
public final class Customer {
    ...
    // ── Business logic ────────────────────────────────────────────────────
    public void suspend() {
        if (status == CustomerStatus.CLOSED) throw new PaymentException.InvalidState("Cannot suspend a closed account");
    ...
    public void requireActive() {
        if (status != CustomerStatus.ACTIVE) throw new PaymentException.CustomerInactive(id, status);
    }
